...
print("201002022 최봉순")
print("2016.12.08 Card_Load_game 0.2")
...
def showxy(event):  #마우스 포인터의 위치를 알려주는 메소드
    ...
    gWin.master.title("Card Load Game ("+str1+")")
...
def stackCheck(event):  # 카드쌓기 순서가 맞는지 체크. 테그 순서를 따라 쌓기진행.
    ...
    #이를 체크리스트에 맞추기위해서 -3을 하였음
        ...
        print('stack length = {}'.format(len(Stackarray)))
        ...
        print('stack length = {}'.format(len(Stackarray)))
        print('value = {}'.format(int(event.widget.find_withtag(CURRENT)[0]-3)))

def onMouseRelease(event):  #클릭 해제시 이벤트
    global last_x, last_y
    if 10<event.x<550 and 10<event.y<490:   
        if event.widget.find_withtag(CURRENT):  #카드창에서 카드클릭해제시 카드
            print('카드')
...
def onRecord():
    fileopen1 = open('c:/tmp/record.txt','r')
    total = fileopen1.readlines()
    root2=Tk()
    root2.title("성적표")
    recWin=Canvas(root2,width=250,height=400)
    recWin.pack()
    reclabel = Label(recWin,text=total)
...
def retrymsg(str1):
    ...
    if result == True:
        ...
        result2 = messagebox.askyesno("Card Load game","종료 하시겠습니까?")
        if result2 == True:
            root.destroy()
